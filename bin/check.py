...
def this_version_string():
    ...
    return json.dumps(
        {
            "python": """{0}-{1}.{2}.{3}-{4}-{5}""".format(
                si, sv.major, sv.minor, sv.micro, sv.releaselevel, sv.serial
            ).lower(),
            "config": str(os.getenv("APP_CONFIG_VERSION", "unknown")),
        },
        indent=None,
        sort_keys=True,
    )
    ...
    for event in events.get("Messages", []):
        try:
            ...
            if event_routing(detail) is True:
                ...
                logger.log(
                    logging.DEBUG if quieter else logging.INFO,
                    " = Deleting SQS ({!s})".format(rh[4:12]),
                )
                quieter = True
        except Exception as e:
...
def handle_ecr_global(input):
    ...
    if input.get("errorCode", "_") in ("ImageAlreadyExistsException"):
        logger.info(
            " = Image already exists ({!s})".format(input.get("errorMessage", ""))
        )
        return True
...
def handle_image(
    ...
    registryaddress = os.environ.get(
        "ECR_REGISTRY_URL_FORMAT", "{regid}.dkr.ecr.{regregion}.amazonaws.com"
    ).format(regid=regid, regregion=regregion)
    imageaddress = "{}/{}".format(registryaddress, repo)
    ...
    logger.debug(
        ' = ECR Credentials: "{}" / "{}" (truncated to 12)'.format(
            registryun[:12], registrypw[:12]
        )
    )

    dc = docker.DockerClient(
        base_url=os.environ.get("DOCKER_DAEMON", "unix://var/run/docker.sock")
    )
    ...
    if hasjava:
        ...
        javaversion.extend(
            [
                "Supports `-XX:MaxRAMPercentage/MinRAMPercentage`: {!s}".format(
                    javacgroupheap
                )
            ]
        )
        if javacgroupheap is False:
            warnings.extend(
                [
                    "**JAVA is too old: {!s} (not support `-XX:MaxRAMPercentage/MinRAMPercentage`)**".format(
                        javaversion[0]
                    )
                ]
            )
            delete = True
            deletereason = "JAVA not support `-XX:MaxRAMPercentage/MinRAMPercentage` {}".format(
                "" if deletereason == "" else " | {}".format(deletereason)
            )
            logger.debug(" = Delete Reason: {}".format(deletereason))
        ...
        try:
            reactivemongo = (
                dc.containers.run(
                    ...
                    log_config={"type": "json-file"},
                    entrypoint="sh",
                    command="-c 'find / -type f -iname \"*reactivemongo_*-*.jar\" | head -n 1 || true'",
                )
                .strip()
                .decode("utf-8")
            )
            ...
            if rmvmatch:
                ...
                if rmvmatch.group(1) == "0" and int(rmvmatch.group(2)) < 13:
                    warnings.extend(
                        [
                            "**REACTIVE-MONGO is too old** ({!s})".format(
    ...
    try:
        ...
        if re.search(r"^v?([0-9]\.|11\.)", nodeversion[0]):
            warnings.extend(
                ["**NODE is too old / EOL / Unstable** ({!s})".format(nodeversion[0])]
            )
    except (docker.errors.ContainerError, docker.errors.APIError) as e:
    ...
    try:
        ...
        if (
            re.search(r"^DEBIAN:[0-8]\.", linuxversion[0])
            or re.search(r"^DEBIAN:9\.[0-7]$", linuxversion[0])
            or re.search(r"^ALPINE:[0-2]\.", linuxversion[0])
            or re.search(r"^ALPINE:3\.[7-8]\.", linuxversion[0])
        ):
            warnings.extend(
                ["**Linux distribution is old** ({!s})".format(linuxversion[0])]
            )
            warnings.extend(
                [
        ...
        if (
            re.search(r"^DEBIAN:[0-7]\.", linuxversion[0])
            or re.search(r"^DEBIAN:8\.([012345J789]|1[0])$", linuxversion[0])
            or re.search(r"^DEBIAN:9\.([01234])$", linuxversion[0])
        ):
            delete = True
            deletereason = "Debian Linux too old {}{}".format(
                linuxversion[0],
                "" if deletereason == "" else " | {}".format(deletereason),
            )
            logger.debug(" = Delete Reason: {}".format(deletereason))
        if re.search(r"^ALPINE:[0-2]\.", linuxversion[0]) or re.search(
            r"^ALPINE:3\.[0-6]\.", linuxversion[0]
        ):
            delete = True
            deletereason = "Alpine Linux too old {}{}".format(
                linuxversion[0],
                "" if deletereason == "" else " | {}".format(deletereason),
            )
            logger.debug(" = Delete Reason: {}".format(deletereason))
    ...
    #                javaversion.extend(
    #                    [
    #                        "**OPENJDK APK package is very out of date:** {!s}".format(
    #                            apkv.strip()
    #                        )
    #                    ]
    #                )
    ...
    try:
        dive = subprocess.run(
            [
                "/usr/local/bin/dive",
                "{}:{}".format(imageaddress, tag),
                "-j",
                "/tmp/dive-{}".format(image.id[7:]),
            ]
        )
        if dive.returncode != 0:
            ...
            with open(
                "/tmp/dive-{}".format(image.id[7:]), "r", encoding="utf-8"
            ) as djf:
    ...
    if dive_json.get("sizeBytes", 0) < 80 * 1024 * 1024:
        logger.debug(
            " = Image is small, skipping efficiency check: {} ({} %)".format(
                dive_json.get("sizeBytes", "unknown"), efficiency
            )
        )
    else:
        ...
        if efficiency < 85:
            warnings.append(
                "**Image build is very inefficient: {} %**".format(efficiency)
            )
        elif efficiency < 90:
    ...
    pretty_who = labels.get(
        "owner_team",
        who[:24] if re.search(r"^[a-zA-Z0-9_\.]+$", "" if who is None else who) else "",
    )

    reportmarkdown = result_text.safe_substitute(
        image=to_md_bullets(
            ["`{}:{}`".format(repo, tag), "_{!s}_".format(image.short_id)]
        ),
        time=eventtime,
        ...
        efficiency=to_md_bullets(
            [
                "{} % (How much disk space is wasted in the image. 100% is best. 0% is worst)".format(
                    efficiency
                )
            ]
        ),
        ...
        if delete is False
            ...
            [
                "{}DELETED ({!s})".format(
                    "*[DRYRUN] Would have been:* " if dryrun else "", deletereason
                )
            ]
        ),
    ...
    at_title = "{}:{}  {}{}".format(
        repo, tag, "" if pretty_who == "" else "@", pretty_who
    )
    at_colour = (
        colours["concern"]
        if (delete and dryrun)
    ...
    at_summary = "{}: {}:{}".format(
        "Deleted" if delete else "Warning" if len(warnings) > 0 else "Accepted",
        repo,
        tag,
    )
    at_msg = (
        "{}**Image deleted** ({})\n\n`{}`".format(
            "*[DRYRUN] Would have been:* " if dryrun else "", deletereason, who
        )
        if delete
        else "**Warning:**\n\n{}".format(to_md_bullets(warnings))
        if len(warnings) > 0
    ...
    if delete and not dryrun:
        try:
            ...
            del_tag = ecrc.batch_delete_image(
                registryId=regid, repositoryName=repo, imageIds=[{"imageTag": tag}]
            )
...
def alert_teams(
    ...
    payload = {
        ...
        "potentialAction": [
            {
                "@type": "OpenUri",
                "name": "Show Full Report (available until {})".format(
                    re.sub(
                        r"(^| )0",
                        r"\1",
